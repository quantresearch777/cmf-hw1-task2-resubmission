...
class Task2SolutionTest(unittest.TestCase):
    ...
    def test_predict_keeps_current_300s_rule_only_for_eth(self):
        trades = frame(
        ...
        output = predict(trades, {}, liq_binance, empty_liq)

        np.testing.assert_array_equal(output[300], np.array([1, 0], dtype=np.int8))
