...
def predict(trades: Any, bbo: Any, liq_binance: Any, liq_bybit: Any) -> dict[int, np.ndarray]:
    ...
    Phase 2 changes only the 300-second decision: the current causal
    liquidation-pressure rule is retained for ETH, while BTC is filtered.
    The 30- and 120-second outputs remain byte-identical to the previous
    submission.
    """
    ...
    eth_only_liq_flags = liq_flags.copy()
    if "ticker" in trades:
        ticker = np.char.lower(_column(trades, "ticker").astype(str))
        eth_only_liq_flags[np.char.find(ticker, "eth") < 0] = 1
    return {
        30: return_flags.copy(),
        120: liq_return_guard_flags.copy(),
        300: eth_only_liq_flags,
    }
